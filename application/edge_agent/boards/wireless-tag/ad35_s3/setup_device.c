 ...
 *
 * AD35-S3 board initialization. Pin assignments were cross-checked against
 * moononournation/AD35-S3's working Arduino examples.
 */
...
#define AW9523_PIN_ES8311_ENABLE   13
#define AW9523_PIN_LCD_RESET       14
#define AW9523_GPIO_MASK ((1U << AW9523_PIN_LCD_LED_0) | \
                           (1U << AW9523_PIN_LCD_LED_1) | \
                           (1U << AW9523_PIN_LCD_LED_2) | \
                           (1U << AW9523_PIN_LCD_LED_3) | \
                           (1U << AW9523_PIN_ES8311_ENABLE) | \
                           (1U << AW9523_PIN_LCD_RESET))
                                       ...
                                       esp_io_expander_handle_t *handle_ret)
{
    ...
    /* Configure both AW9523 ports as push-pull GPIO instead of LED sinks. */
    uint8_t data = 0x10;
    ESP_RETURN_ON_ERROR(esp_io_expander_aw9523b_write_reg(*handle_ret, AW9523B_REG_GCR, &data, 1), TAG,
                        "AW9523B push-pull configuration failed");
    data = 0xFF;
    ESP_RETURN_ON_ERROR(esp_io_expander_aw9523b_write_reg(*handle_ret, AW9523B_REG_LEDMODE0, &data, 1), TAG,
                        "AW9523B port 0 GPIO-mode configuration failed");
    ESP_RETURN_ON_ERROR(esp_io_expander_aw9523b_write_reg(*handle_ret, AW9523B_REG_LEDMODE1, &data, 1), TAG,
                        "AW9523B port 1 GPIO-mode configuration failed");
    ESP_RETURN_ON_ERROR(esp_io_expander_set_dir(*handle_ret, AW9523_GPIO_MASK, IO_EXPANDER_OUTPUT), TAG,
                        "AW9523B output-direction configuration failed");

    /* LCD enables are active-low; codec enable is active-high. */
    ESP_RETURN_ON_ERROR(esp_io_expander_set_level(*handle_ret,
                        (1U << AW9523_PIN_LCD_LED_0) | (1U << AW9523_PIN_LCD_LED_1) |
                        (1U << AW9523_PIN_LCD_LED_2) | (1U << AW9523_PIN_LCD_LED_3) |
                        (1U << AW9523_PIN_LCD_RESET), 0), TAG, "AW9523B LCD initial state failed");
...
static int display_lcd_init(void *config, int cfg_size, void **device_handle)
{
    (void)config;
    (void)cfg_size;
    ESP_RETURN_ON_FALSE(device_handle != NULL, ESP_ERR_INVALID_ARG, TAG, "device_handle is NULL");

    esp_io_expander_handle_t *expander = NULL;
    ESP_RETURN_ON_ERROR(esp_board_device_get_handle("gpio_expander", (void **)&expander), TAG,
                        "AW9523B handle unavailable");
    ESP_RETURN_ON_ERROR(esp_io_expander_set_level(*expander, 1U << AW9523_PIN_LCD_RESET, 0), TAG,
                        "LCD reset assert failed");
    vTaskDelay(pdMS_TO_TICKS(200));
    ESP_RETURN_ON_ERROR(esp_io_expander_set_level(*expander, 1U << AW9523_PIN_LCD_RESET, 1), TAG,
    ...
    esp_lcd_i80_bus_config_t bus_cfg = {
        ...
        .data_gpio_nums = {LCD_PIN_D0, LCD_PIN_D1, LCD_PIN_D2, LCD_PIN_D3,
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_new_i80_bus(&bus_cfg, &s_i80_bus), TAG, "i80 bus creation failed");
    ...
    ESP_RETURN_ON_ERROR(esp_board_device_override_config("display_lcd", (void *)&s_lcd_config,
                                                          sizeof(s_lcd_config)), TAG,
                        "display configuration override failed");
    *device_handle = &s_lcd_handles;
    ESP_LOGI(TAG, "AD35-S3 ST7796 LCD ready (%dx%d)", LCD_H_RES, LCD_V_RES);
    return ESP_OK;
}
...
static int display_lcd_deinit(void *device_handle)
{
    ...
    cleanup_display_lcd(handles ? handles->panel_handle : NULL, handles ? handles->io_handle : NULL);
